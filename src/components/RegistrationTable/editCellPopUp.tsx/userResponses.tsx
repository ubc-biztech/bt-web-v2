...
// Row is needed to know which user to make the query for
// Likely will need to know which event to query for as well
interface EditCellProps {
    row: Row<Attendee>,
}

const UserResponses: React.FC<EditCellProps> = ({ row }) => {
    ...
    useEffect(() => {
        if (router.isReady) {
            ...
            if (eventId && attendeeId) {
                getQAData(attendeeId, eventId).then(d => {
                    setData(d)
                })
            }
        }
    },[router.isReady, router.query.eventId, row.original.id]);
...
async function getQAData (attendeeId: string, eventId: string | string[]) {
    // TODO: Handle query logic here
    // My frontend code is configured to work with a dictionary of question keys and answer values, 
    // but can be open to change depending on how data is retrieved
    const qaDict =
    {
