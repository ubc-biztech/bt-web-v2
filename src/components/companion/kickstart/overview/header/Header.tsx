...
const Header = ({ teamName, setPage, modal }: { teamName: string, setPage: (arg0: KickstartPages) => void, modal: boolean }) => {
  return (
    <div className="w-full h-30 flex flex-col items-center jusitfy-center md:mt-14 sm:mt-6">
      ...
      <div className="w-full flex flex-row items-end justify-between text-[24px]">
        <header className={`font-instrument text-[42px] flex items-end leading-none ${modal ? "text-[#DE7D02]" : "text-white"} transition duration-500 ease-in-out`}>
          {teamName} <span className={`transition duration-200 ease-in-out text-white ml-2 ${modal ? "opacity-100" : "opacity-0"}`}>Overview</span>
        </header>
        <div className="md:flex hidden">
          <Ticker />
        </div>
        <div onClick={() => {setPage(KickstartPages.INVEST)}} >     
