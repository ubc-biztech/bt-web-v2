...
const ProjectBox: React.FC<ProjectBoxProps> = ({ res, presenting, showRubric, setSelectedTeam }) => {
  return (
    <div className='w-full h-32'>
      <Box width={100} height={20} fitToParent={true} className={`flex flex-row justify-between items-center pl-5`}>
...
const Rounds: React.FC<RoundsProps> = ({ feedback }) => {
  const [currentRound, setCurrentRound] = useState(Math.max(...Object.keys(feedback).map((key) => parseInt(key, 10)))); //may not be true
  ...
  useEffect(() => {
    setCurrentRound(Math.max(...Object.keys(feedback).map((key) => parseInt(key, 10))));
  }, [feedback]);
    ...
    const fetchCurrentTeam = async () => {
      try {
        ...
        if (response?.currentTeamName) {
          ...
          setTeamData({ ...TeamData, teamID: response.currentTeamID });
      ...
      <FadeWrapper className='flex flex-row mt-10 gap-8'>
        <div className='w-36 flex flex-col gap-5'>
          {Object.keys(feedback).map((round, index) => (
            <CornerBorderWrapper selected={round === currentRound.toString()} key={index}>
              <div
              ...
              >
                <span className={`text-md ${round === currentRound.toString() ? "text-white" : "text-[#656795]"}`}>
                  ROUND {round}
        ...
        {!noData && (
          <div className='w-full flex flex-col gap-5'>
            {currentTeam && (
              <ProjectBox
                res={{
                  teamID: (currentTeam as any).currentTeamID || "",
                  team: (currentTeam as any).currentTeamName || "",
            ...
            {Object.keys(feedback).map((round: string, index: number) => {
              return feedback[round].map((team: Team, sIndex: number) => {
                ...
                const scored = Object.values(team.scores).every((score: Score) => parseInt(score.N) != 0);
