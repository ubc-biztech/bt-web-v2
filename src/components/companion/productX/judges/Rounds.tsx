...
const Rounds: React.FC<RoundsProps> = ({ records }) => {
    ...
    const teamsJudged = Object.values(records).flat();

    // checking if data exists
    if (Object.keys(records).length === 0 || teamsJudged.length <= 0) {
        return (
            <div className="flex flex-col items-center justify-center w-full min-h-[600px] border-2 border-dashed border-[#41437D] p-8">
                ...
                <span className="pt-2 text-[#656795] text-center max-w-[600px] text-sm">
                    ...
                    you&apos;ll see an option to begin scoring their
                    project.&quot;
                </span>
            </div>
        );
    ...
    const [currentRound, setCurrentRound] = useState(
        Math.max(...Object.keys(records || {}).map((key) => parseInt(key, 10)))
    );
            ...
            try {
                ...
                if (response?.currentTeamName) {
                    ...
                    const searchFeedback = teamsJudged.find(
                        (feedback) => feedback.teamID === response.currentTeamID
                    );
                    if (searchFeedback)
                        setTeamFeedback(searchFeedback);
                    else {
        ...
        <>
            <FadeWrapper className="flex flex-row mt-10 gap-8">
                <div className="w-full flex flex-col gap-5">
                    ...
                    {teamsJudged.map(
                        (teamFeedback: TeamFeedback, index: number) => {
                            ...
                            return (
                                scored && (
                                    <ProjectRow
                                        ...
                                        team_status={`COMPLETED ${formatDate(
                                            teamFeedback.createdAt
                                        )}`}
                                        read_only={false}
                                        presenting={false}
                                        onClick={() => {
