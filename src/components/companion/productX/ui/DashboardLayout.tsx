...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ title, pages, teams }) => {
  const [currentlyDisplayedPage, setCurrentlyDisplayedPage] = useState(pages[0].name);
  return (
    <div className='w-full px-10'>
      <div className='flex flex-col'>
        ...
        <div className='border-b-2 border-[#41437D] mt-6 flex flex-row'>
          {pages.map((page, idx) => {
            ...
            return (
              ...
              >
                <page.icon size={16} color={isActivePage ? "#4CC8BD" : "#41437D"} />
                {page.name}
              </div>
            );
          })}
        ...
        {pages.map((page) => {
          return currentlyDisplayedPage === page.name && <page.component records={teams} />;
        })}
      </div>
    </div>
  );
