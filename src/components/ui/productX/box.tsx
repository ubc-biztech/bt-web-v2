...
const Box: React.FC<BoxProps> = ({
  ...
  const defaultBoxStyles = defaultEffects ? "border-2 border-[#1B1C39] bg-[#020319]" : "";

  const hoverBoxStyles = hoverEffects && !selectableEffects ? "border-2 border-[#1B1C39] bg-[#020319]" : "";

  const selectableBoxStyles = selectableEffects
    ? `border-2 ${isSelected ? "border-[#23655F] text-[#4CC8BD]" : "border-[#1B1C39]"} bg-[#020319]`
    : "";
  ...
  const boxShadowStyle = () => {
    if (defaultEffects) {
      ...
      return {
        boxShadow: `inset 0 0 ${shadowSize}px rgba(${isHovered ? 203 : 39}, ${isHovered ? 204 : 40}, ${isHovered ? 233 : 76}, ${
          isHovered ? 0.5 : 1
        })`,
        transition: "filter 0.4s cubic-bezier(0.4, 0, 0.2, 1), box-shadow 0.3s ease-in-out"
      };
