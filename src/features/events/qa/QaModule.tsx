...
export function QaModule({ eventId, year, isAdmin, signedIn }: QaModuleProps) {
  ...
  return (
    <section className="flex min-h-[275px] flex-col overflow-hidden rounded-lg border border-[#263451] bg-[#0B152C] shadow-[0_12px_28px_rgba(0,0,0,0.2)]">
      <div className="flex flex-1 flex-col gap-4 p-5 md:p-6">
        ...
        {/* Featured question */}
        {isLoading ? (
          <div className="h-20 animate-pulse rounded-md bg-[#151515]" />
        ) : featured ? (
      ...
      <AskQuestionModal
        ...
        signedIn={signedIn}
        onViewBoard={() => {
          window.location.href = `/event/${eventId}/${year}/qa`;
        }}
      />
    </section>
  );
