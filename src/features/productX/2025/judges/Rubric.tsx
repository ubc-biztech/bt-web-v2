...
import { capitalizeTeamName } from "../utils";
...
const Rubric: React.FC<RubricProps> = ({
  ...
  const [score, setScore] = useState<ScoringRecord>(
        ...
        <div className="w-full flex flex-row items-center justify-between mb-56 mt-12">
          <div className="flex flex-col text-[#898BC3] gap-2">
            <span className="text-lg text-white">
              TOTAL SCORE:&nbsp;
              {metrics.every((metric) => score[metric] !== undefined)
                ? metrics.reduce((total, metric) => total + score[metric], 0)
                : "N/A"}
            </span>
            {metrics.map((metric) => (
              <span
                key={metric}
              >{`${mapMetricsToCategories[metric]}: ${score[metric] || "N/A"}`}</span>
            ))}
          </div>
            ...
            <Button
              ...
              onClick={() => {
                const allScored = metrics.every(
                  (metric) =>
                    typeof score[metric] === "number" && score[metric] > 0,
                );
