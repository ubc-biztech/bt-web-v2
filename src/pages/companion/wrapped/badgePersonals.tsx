    ...
    useEffect(() => {
        const fetchBadges = async () => {
            try {
                ...
                const processedBadges = response.data.map((badge: Omit<Badge, "isComplete">) => ({
                    ...badge,
                    isComplete: badge.progress >= badge.threshold
                }));

                // Filter only completed badges
                const completed = processedBadges.filter((badge: Badge) => badge.isComplete);
                ...
                setPageError(err instanceof Error ? err.message : "Failed to load badges.");
            }
        };
                        ...
                        <div className="grid grid-cols-5 gap-3 justify-center">
                            {completedBadges.map((badge) => (
                                <motion.div
                                    ...
                                    transition={{ delay: 0.05 * completedBadges.indexOf(badge), duration: 0.3, ease: "easeOut" }}
                                >
