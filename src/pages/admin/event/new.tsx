...
const CreateEventPage: NextPage = () => {
    ...
    const handleSubmit = async (data: EventFormSchema) => {
        ...
        data.customQuestions.forEach(q => {
            if (q.type === "WORKSHOP_SELECTION" && q.options.length !== q.participantCap) {
                toast({
                    title: "Validation Error",
                    description: "Workshop participant caps must match number of workshops",
                    variant: "destructive",
                });
                return;
            }
        ...
        const transformCustomQuestion = (q: any) => ({
            ...
            participantCap: q.type === 'WORKSHOP_SELECTION' ? q.participantCap : undefined,
        ...
        const body = {
            ...
            partnerRegistrationQuestions: data.partnerCustomQuestions.map(transformCustomQuestion),
        ...
        try {
            ...
            router.push(`/admin/event/${data.eventSlug}/${data.startDate.getFullYear()}/edit`);
        } catch (error: any) {
            console.log(error);
            if (error.status === 409) {
                toast({
